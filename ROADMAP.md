...
| 2026-07-08 (s3) | **Benchmark Steps 3–6 done — the pre-model scoreboard is committed.** Floor baselines (PWM Δ-score, logistic-on-motifs) + AlphaGenome (API, 1MB context) scored on the identical held-out set, per context, with bootstrap CIs. **Off-wedge (K562/HepG2): AlphaGenome 0.65–0.68, beats floors** (leakage caveat: those lines are in its training data). **On-wedge pooled (n=58): no model significantly above chance** — AlphaGenome 0.552 [.414,.690], logistic 0.534, PWM 0.500, all CIs span 0.5 and are not separable. (AlphaGenome first mis-scored via an rsID-keying bug on 5 multi-allelic variants → rescored per variant_id; on-wedge 0.500→0.552, still not sig>chance.) rs6733839 stays a directional miss in immune/microglia. The wedge is real and open. Borzoi/Flashzoi row = pending GPU (`benchmark/borzoi_run.py` committed, NOT faked — no GPU connected). Commits: floor 05ff49d5, AlphaGenome 25e3ae6f→corrected 729f3b28, scoreboard 1640a6fa→corrected 729f3b28. | **Next fork: design + train the directional wedge model (needs a GPU host connected). Definition of a win: on-wedge pooled accuracy significantly above 0.5, CI clearing both the incumbent and the floor.** |
