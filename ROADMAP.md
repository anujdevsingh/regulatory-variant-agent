...
| 2026-07-08 (s2) | **Predicted the effect**: Berg–von Hippel affinity from the MEF2 PWM → risk T ~190× tighter MEF2A binding (λ=1; occupancy shift shown). Four readouts agree in direction — three predictions (motif/BvH, AlphaGenome, ChromBPNet) + one measured (brain MPRA, PMC12265656, cross-checked in results/MPRA_VALIDATION.md). Staged Boltz-2 co-fold (ref vs alt) — needs a GPU host to run. | Scientific result upgraded toward STRONG. **Next: user chose to consider route #2 (train our own model) next.** |
